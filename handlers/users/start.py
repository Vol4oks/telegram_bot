...
# Когда пользователь преходит по ссылке
# Тогда по нажатию на кнопку start - боту приходит команда с аргументом 123
...
async def bot_start_deeplink(message: types.Message):
    # С помощью функции get_args забираем аргументы после команды start.
    # (для примера выше - будет "123")
    deep_link_args = message.get_args()

    await message.answer(f'Привет, {message.from_user.full_name}!\n'
                         f'Вы находитксь в личной переписке. \n'
